    ...
    return (
        <div className="flex flex-col h-screen bg-slate-900 text-slate-200 font-sans antialiased">
            <style>{`
                ...
                .markdown-content .hljs-keyword { color: #c792ea !important; }
                .markdown-content .hljs-string { color: #c3e88d !important; }
                .markdown-content .hljs-number { color: #f78c6c !important; }
                .markdown-content .hljs-comment { color: #546e7a !important; }
                .markdown-content .hljs-function { color: #82aaff !important; }
                .markdown-content .hljs-variable { color: #eeffff !important; }
                .markdown-content .hljs-built_in { color: #ffcb6b !important; }
                .markdown-content .hljs-tag { color: #f07178 !important; }
                .markdown-content .hljs-attr { color: #c792ea !important; }
                .markdown-content .hljs-title { color: #82aaff !important; }
                    ...
                    <div className="max-w-4xl mx-auto space-y-8 pb-4">
                        {chatHistory.length === 0 && !isGenerating && (
                            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center fade-in">
                                <div className="welcome-container rounded-3xl p-8 mb-8 max-w-2xl">
                                    <div className="flex items-center justify-center mb-6">
                                        <div className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center">
                                            <Brain size={32} className="text-white" />
                                        </div>
                                    </div>
                                    <h2 className="text-3xl font-bold mb-4 gradient-text">مرحباً بك في {danName}</h2>
                                    <p className="text-slate-400 text-lg mb-8">مساعدك الذكي جاهز للإجابة على أسئلتك ومساعدتك في مختلف المهام</p>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                        {WELCOME_SUGGESTIONS.map((suggestion, index) => (
                                            <button
                                                ...
                                                className="suggestion-card p-4 rounded-xl text-right hover:scale-105 transition-all duration-300"
                                            >
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center flex-shrink-0">
                                                        {index % 4 === 0 && <Lightbulb size={16} className="text-white" />}
                                                        {index % 4 === 1 && <MessageCircle size={16} className="text-white" />}
                                                        {index % 4 === 2 && <Zap size={16} className="text-white" />}
                                                        {index % 4 === 3 && <Sparkles size={16} className="text-white" />}
                                                    </div>
                                                </div>
                                            </button>
                                        ))}
                                    ...
                                    <p className="text-slate-500 text-sm">💡 نصيحة: استخدم "/" لاستكشاف الأوضاع المختلفة</p>
                                </div>
                            </div>
                        )}
            ...
            <footer className="relative w-full px-4 md:px-8 bg-slate-900/70 backdrop-blur-xl">
                ...
                {editingMessageId && (
                    <div className="max-w-4xl mx-auto bg-indigo-900/20 border border-indigo-500/30 rounded-lg p-3 mb-2 fade-in-up">
                        <div className="flex items-center gap-2 text-indigo-400 text-sm mb-2 justify-end">
                        ...
                        <div className="flex gap-2 justify-end">
                            ...
                            <button 
                                onClick={handleSaveUserEditAndRegenerate} 
                                className="px-3 py-1 text-sm bg-indigo-600 hover:bg-indigo-500 rounded-md text-white"
                            >
                                حفظ وإرسال
                            </button>
                        </div>
                ...
                <form onSubmit={editingMessageId ? handleSaveUserEditAndRegenerate : handleSendMessage} className="max-w-4xl mx-auto flex items-end gap-2 p-2 pt-3 pb-4">
                    <div className="flex-1 relative bg-slate-800 rounded-2xl border border-slate-700/50 focus-within:ring-2 focus-within:ring-indigo-500 transition-all shadow-lg">
                         <textarea ref={inputRef} value={userInput} onChange={handleUserInput} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); editingMessageId ? handleSaveUserEditAndRegenerate() : handleSendMessage(e); } }} placeholder={editingMessageId ? "تعديل الرسالة..." : "اسأل أي شيء... أو استخدم /"} className="w-full bg-transparent p-3 pl-12 resize-none outline-none text-slate-200 placeholder:text-slate-500 disabled:opacity-50" dir="auto" rows="1" onInput={(e) => { e.target.style.height = 'auto'; e.target.style.height = `${e.target.scrollHeight}px`; }} disabled={isGenerating}/>
                         {image && (
                            <div className="absolute -top-20 right-0 p-1 bg-slate-700/80 rounded-lg"><img src={image.data} alt="Preview" onClick={() => setFullscreenImage(image.data)} className="h-16 w-16 object-cover rounded cursor-pointer" /><button onClick={() => setImage(null)} className="absolute top-0 left-0 -mt-2 -ml-2 bg-slate-800 rounded-full text-slate-400 hover:text-white"><XCircle size={20} /></button></div>
                        )}
                         <button type="button" onClick={handleMicClick} title="إدخال صوتي" className={`absolute left-2 top-1/2 -translate-y-1/2 p-2 transition-colors ${isRecording ? 'text-red-500 animate-pulse' : 'text-slate-400 hover:text-indigo-400'}`}><Mic size={20} /></button>
                    </div>
...
function MessageBubble({ msg, danName, onReply, onEdit, onRegenerate, onContinue, onCopy: handleCopy, isGenerating, editingMessageId, isLastUserMessage }) {
    ...
    useEffect(() => {
        if (contentRef.current && window.marked && !msg.isThinking) {
        ...
        if (msg.isThinking) {
            return (
                <div className="flex items-center gap-3 text-slate-400 py-2">
                    <div className="w-6 h-6 rounded-full thinking-pulse flex items-center justify-center">
                        <div className="w-2 h-2 bg-white rounded-full"></div>
                    </div>
                    <span className="text-base">جارٍ التفكير...</span>
                </div>
            );
        }
        return (
            <>
                ...
                <div 
                    ref={contentRef}
                    className={`markdown-content ${!isUser ? 'text-base leading-relaxed' : ''}`}
                    dir="auto" 
                />
            </>
        );
             ...
             <div className={`flex flex-col ${isUser ? 'items-end max-w-xl' : 'items-start w-full'} min-w-0`}>
                <div
                    ...
                    className={`break-words w-full transition-all
                        ...
                        ${!isUser && !msg.isThinking ? 'blurry-fade-in-effect' : ''}
                    `}
                >
                    {renderDisplayContent()}
                </div>
                 <div className={`flex items-center gap-3 text-slate-500 transition-opacity duration-300 mt-2 ${isUser ? 'self-end' : 'self-start'} ${(msg.isThinking || !msg.content) ? 'opacity-0' : 'opacity-100'}`}>
